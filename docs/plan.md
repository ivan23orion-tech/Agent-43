...
|-----------------|--------------------------------------------------------------------------------------------------------------------------|-------------|
| **Task**        | `id` (UUID, PK), `title` (string), `description` (texto), `created_at` (timestamp), `creator_id` (referência ao usuário, opcional), `status` (enum: `open`, `closed`, `in_review`, etc.), `rewardAmount` (inteiro, opcional), `rewardCurrency` (enum `USDT`/`USDC`, opcional) | Representa a tarefa criada.  O campo `status` controla se a tarefa aceita submissões; `rewardAmount` e `rewardCurrency` estruturam tarefas pagas em stablecoins. |
